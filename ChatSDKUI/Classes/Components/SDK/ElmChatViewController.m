...
-(void) setupTextInputView {
  ...
  _sendBarView.backgroundColor = UIColor.clearColor;
...
- (void)viewDidLoad {
  ...
  tableInsets.left += 12;
  tableInsets.right += 12;
  tableView.contentInset = tableInsets;
  
  UIColor * lightGrey = [UIColor colorWithRed:0.945 green:0.956 blue:0.968 alpha:1];
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  id<PElmMessage> message = [self messageForIndexPath:indexPath];
  return false;//![message.userModel isEqual:NM.currentUser];
}
