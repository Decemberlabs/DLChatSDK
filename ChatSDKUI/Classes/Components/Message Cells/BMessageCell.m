...
-(instancetype) initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
  ...
  if (self) {
    ...
    UIColor * hlyRed = [UIColor colorWithRed:0.75 green:0.19 blue:0.25 alpha:1];
    
    _profilePicture = [[UIImageView alloc] init];
    _profilePicture.clipsToBounds = YES;
    _profilePicture.layer.borderWidth = 2;
    _profilePicture.layer.borderColor = hlyRed.CGColor;
...
-(void) setMessage: (id<PElmMessage>) message withColorWeight: (float) colorWeight {
  ...
  if (position & bMessagePosLast) {
    ...
    _profilePicture.layer.borderWidth = 0;
